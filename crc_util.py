...
class CRC:
    ...
    def __init__(self):
        self.FCSTAB = [0x0000, 0x1189, 0x2312, 0x329b, 0x4624,
        ...
        self.FCS_START = 0xffff

    def get_fcs(self, fcs, s):
        return (fcs >> 8) ^ self.FCSTAB[(fcs ^ s) & 0x00ff]

    def get_crc(self, hex_str):
        hex_str = re.sub(" ", "", hex_str)
        hex_str = re.sub(r"(.{2})", r"\1,", hex_str)
        str_list = [s for s in hex_str.split(",") if len(s) > 0]
        crc = self.FCS_START
        for s in str_list:
            parse_int = int(s, 16)
            crc = self.get_fcs(crc, parse_int)
        crc ^= self.FCS_START
        return "{:04X}".format(crc)

    def make_crc(self, psrc, psrc_len):
        """
        :param psrc: byte array of data
        :param psrc_len: length of byte array
        :return: hex string of crc (capital letter)
        example input: [0xba, 0x8a]
        example output: "84B4"
        """
        crc = self.FCS_START
        for i in range(psrc_len):
            crc = self.get_fcs(crc, psrc[i])
        crc ^= self.FCS_START
...
if __name__ == "__main__":
    ...
    s = 175
    crc= CRC()
    print(crc.get_fcs(fcs, s))
    print(crc.get_crc("a4867946"))

    psrc = bytes([0xba, 0x8a, 0x0d, 0x45, 0x25, 0xad, 0xd0, 0x11,
            0x98, 0xa8, 0x08, 0x00, 0x36, 0x1b, 0x11, 0x03])
    print(psrc)
    print(crc.make_crc(psrc, len(psrc)))
    # print(unhexlify(hex(33972)[2:]))
